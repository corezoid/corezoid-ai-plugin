...
- Markdown link check.
- Skills-list sync (`scripts/check-skills-sync.py`): the skill directories under `plugins/corezoid/skills/` must match the lists in `CLAUDE.md`, `README.md`, and `POWER.md`.
...
scripts/
  ...
  check-skills-sync.py            — CI check: skills directories vs CLAUDE.md vs README.md vs POWER.md skill lists
public/
...
`tools/list` advertises 18 core tools (`push-process`, `pull-process`, `lint-process`, `run-task`, `login`, …) plus seven **router tools** — `cz-access`, `cz-structure`, `cz-tasks`, `cz-dashboards`, `cz-variables`, `cz-snapshots`, `cz-git-context` — that front the 54 CRUD-shaped tools, called as `cz-access {"action": "share-object", "args": {…}}`. This exists because the list ships as one line on stdio against a 64 KiB budget and costs context in every session; collapsing the CRUD domains took it from 65 400 to 35 900 bytes.
...
When adding a new skill, also update this file's Architecture section, the skills table in `README.md`, and the Kiro skill list in `POWER.md`. CI (`scripts/check-skills-sync.py`) fails the build if any list diverges from the actual directory.
