...
Golden tests (layout coordinates in `testdata/golden/layout_*.json`, lint output in `testdata/golden/lint_*.txt`, the `tools/list` payload in `testdata/golden/tools_list.json`) are regenerated with the `-update` flag after an intentional change:
...
scripts/
  ...
  check-version-sync.py           — Release check: the version in the four manifests vs the MCP server's fallbackServerVersion
public/
...
- `plugins/corezoid/mcp-server/testdata/golden/tools_list.json` — the serialized `tools/list` payload. Regenerate with `go test -run TestToolsList_Snapshot -update ./...` whenever a tool's name, description, annotations, input schema, or registry position changes.

Tool definitions live in per-domain `plugins/corezoid/mcp-server/tools_*.go` files mirroring the `mcp_handlers_*.go` split; `tools_registry.go` only concatenates them. A new tool needs an entry in its domain file (with one of the annotation presets from `tools_annotations.go`), a handler in `toolHandlers`, a row in the README MCP-tools table, and a regenerated `tools_list.json`.
