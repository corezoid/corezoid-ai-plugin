...
- **1-to-1 rule:** Each failing node must have its own **unique** `err_node_id` — never share a single Reply+Final Error pair across multiple source nodes. A shared error node makes it impossible to diagnose which step failed.
- **Subprocess error rule:** If the process is invoked via `api_rpc` (i.e. it is a subprocess called with "Call a Process"), every individual error path **must** insert a dedicated `api_rpc_reply` node (`throw_exception: true`) immediately before its own Final Error node. The chain is: `failing_node → [err_node_id] → api_rpc_reply (throw_exception:true) → [go] → Final Error (obj_type:2)`. Each such chain must be unique per failing node — titles must reflect the specific step (e.g. "Reply: Invoice generation error", "Error: Invoice generation" are valid only when they belong exclusively to one source node).
