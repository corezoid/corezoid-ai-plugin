...
var toolRegistry = []mcpTool{
	...
	{
		Name:        "lint-process",
		Description: "Validate process structure locally. Reports orphaned/no-op nodes, broken links, reply and old-format problems, timer/API shape issues, active Stub Mode, and advisory git_call usage constraints. When the opt-in project policy is enabled, it also proves reachable cycle bounds, reports static/unresolved process recursion risk, and validates typed/described process input/output contracts plus statically resolved callee mappings. Warn mode reports only; strict findings are enforced by push-process.",
