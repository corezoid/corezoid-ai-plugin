		...
		InputSchema: map[string]interface{}{
			...
			"properties": map[string]interface{}{
				...
				"force": map[string]interface{}{
					"type":        "boolean",
					"description": "Deploy despite generic blocking lint findings or overwrite a concurrent server change. Does not confirm active Stub Mode; use allow_active_stub_mode for that. Does not waive undeclared logic properties; use allow_unknown_logic_props for that. Advisory findings never block. Does NOT bypass pre-deployment validation errors such as self-referencing api_copy/api_rpc nodes — those must be fixed in the process design. Default false.",
				},
				...
				"allow_unknown_logic_props": map[string]interface{}{
					"type":        "boolean",
					"description": "Explicitly allow deploying nodes that carry a logic/semaphor property no schema declares. Use only for a field Corezoid added to a deployed node (it arrived via pull-process and must round-trip unchanged) — never to silence a typo, which is what this gate exists to catch since a misspelled property is in no required list. Deliberately separate from force, which would also waive the concurrent-change gate. Default false.",
				},
