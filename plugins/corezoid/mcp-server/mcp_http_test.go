...
func TestAuthWrap_NoToken_PassThrough(t *testing.T) {
	os.Unsetenv("COREZOID_HTTP_AUTH_TOKEN")
	inner := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
	})
	handler := authWrap(inner)
	...
	if w.Code != http.StatusOK {
		t.Errorf("expected 200 when no auth token configured, got %d", w.Code)
	}
}

func TestAuthWrap_WithToken_Correct(t *testing.T) {
	os.Setenv("COREZOID_HTTP_AUTH_TOKEN", "secret123")
	t.Cleanup(func() { os.Unsetenv("COREZOID_HTTP_AUTH_TOKEN") })
...
func TestAuthWrap_WithToken_Wrong(t *testing.T) {
	os.Setenv("COREZOID_HTTP_AUTH_TOKEN", "secret123")
	t.Cleanup(func() { os.Unsetenv("COREZOID_HTTP_AUTH_TOKEN") })
...
func TestAuthWrap_WithToken_Missing(t *testing.T) {
	os.Setenv("COREZOID_HTTP_AUTH_TOKEN", "secret123")
	t.Cleanup(func() { os.Unsetenv("COREZOID_HTTP_AUTH_TOKEN") })
...
func TestAuthWrap_Options_AlwaysAllowed(t *testing.T) {
	os.Setenv("COREZOID_HTTP_AUTH_TOKEN", "secret123")
	t.Cleanup(func() { os.Unsetenv("COREZOID_HTTP_AUTH_TOKEN") })
