...
- Feat(skills): `corezoid-gen-bot` and `corezoid-edit-bot` turn a set of existing Corezoid processes into a live multi-platform messenger bot and keep it maintainable. `gen-bot` runs `plan` → `execute` → `refresh`: it pulls every handed-in process and derives its real input/output contract from its `api_rpc_reply` nodes rather than trusting the description, designs a command map that uses all of them, then creates a Communications Orchestrator, generates one bot process per command wired over `api_rpc`, seeds Localization and Attachments, pushes, lints and smoke-tests every command. Phase 4 — the wizard call — always waits for an explicit confirmation: the skill previously said both "pause and wait" and "auto-proceed when these five conditions hold", and an irreversible step that mints ~150 processes cannot be left to whichever reading a given run picks. `edit-bot` owns the follow-up changes — add, rename or drop a command, wire another process, change copy or keyboards, promote a stage — through plan → confirm → apply → lint → push → smoke-test, because an edit lands on a bot people are already talking to and most of the ways it breaks are silent: nothing fails at push time, the chat just misbehaves. Both skills are verified executable as written, because all three ways they were not failed only after the irreversible step: `pull-folder` is called with the stage id (the zero-argument form exists only in the server's CLI mode and is refused over MCP), the skeletons' `parent_id` and `api_copy.user_id` placeholders are substituted unquoted so the first push of every generated command is not a schema-validation repair loop, and `references/` paths resolve through `${CLAUDE_PLUGIN_ROOT}`.
- Feat(mcp-server): new `create-communications-orchestrator` tool builds a multi-platform messenger robot — Telegram, Facebook Messenger, Viber and Apple Messages for Business — from one `messengers` list, at least one channel. Corezoid builds it asynchronously, so the tool queues the `bot_wizzard` job and then polls its status every 3 seconds up to 10 times, reporting success only once the wizard hands back the `folder_url` of the generated folder: a queued job can still fail on a rejected channel token, and "created" without the folder link is indistinguishable from a wizard that built nothing. A failed build returns the wizard's own diagnosis (which token it rejected) rather than a generic error, a build still running when the budget runs out returns the job id so it can be traced instead of duplicated, and a transient status-check failure is retried rather than reported as a failed build. The tool is annotated as destructive even though the build only adds objects: the annotation is what makes a host ask before running it, and this call has no undo — a second build against a channel token that already serves a bot takes that bot's webhook over, breaking a working integration without deleting anything. The details stay in the skill rather than the tool description, because the whole tool list ships as one ~65 KB line and a 64 KiB-buffered reader (bufio.Scanner, asyncio's default StreamReader) drops it; a test now bounds that line so the next added description fails in CI with the reason instead of surfacing as a dead session. Every channel credential is masked in the debug log, by `*_token`/`*_secret` suffix rather than by an exhaustive field list, so a channel added later is covered by default. A supplied `stage_id`/`project_id` must be a whole number greater than zero, and an ABC `user_id` likewise: the shared integer reader truncates a JSON float and reports an unparseable value the same way it reports an absent key, so `12345.6` would have built the bot in stage `12345` and `"abc"` in whatever stage the workspace had configured — irreversibly, and in neither case where the caller said. Malformed channel entries — unknown channel, missing or misspelled token field, a channel listed twice — are rejected locally, because the API answers all of them with a `Value is not valid` that does not say which entry is wrong.
- Feat(mcp-server): `run-task`, `create-snapshot`, `list-snapshots`, `delete-snapshot` and `get-snapshot` identify their target by `process_id`, not only by `process_path`. They needed a local file for one reason — to read the numeric id out of its `<ID>_<name>.conv.json` name — so a host with no local process repository (the Simulator.Company AI console) could not run a task at all without first pulling the process. `process_id` is the same argument name `show-task`/`list-task-history`/`pull-process` already take; `process_path` keeps working unchanged, passing both is refused as ambiguous rather than resolved by picking one (several of these calls delete things), and a `process_id` that is zero, negative or fractional is refused instead of truncated. The advertised JSON Schema declares the pair as `anyOf` with both properties nullable, because the runtime treats `""` and `null` as "not supplied" — a host that fills unset optionals rather than omitting them would otherwise have its call rejected client-side, on presence or on type, for a request the server accepts. The plugin manifests also gained an `interface` block (display name, capabilities, sample prompts) so the plugin is described consistently in host UIs.
...
- Fix(discovery): `public/llms.txt` and the skills index list every MCP tool, read from `tools_registry.go` instead of a list maintained by hand in `scripts/generate-discovery.py`. That list had drifted to 18 of 71 tools, so the machine-readable index an agent uses to find out what this plugin can do omitted whole feature areas — snapshots, groups, layout, stage deploys, `create-communications-orchestrator` — and no amount of regeneration fixed it, because the staleness was in the generator. It now fails loudly if the registry stops parsing rather than publishing an empty tool list.
- Docs: the two bot skills are discoverable. `corezoid/SKILL.md` — the documented entry point that routes to the sub-skills — did not list them, and neither the interface blocks nor the keywords of any of the three plugin manifests said anything about bots, messengers, Telegram or chat, so a host UI describing this plugin would advertise 3.3.0’s feature set. Each manifest gained a bot capability, a sample prompt and a `longDescription` clause, plus `chatbot`/`telegram` keywords mirrored into the marketplace manifest's keywords and description.
- Build: `github.com/gofrs/flock` 0.13.0 → 0.13.1.
...
- Fix(mcp-server): `allow_no_snapshot` no longer bypasses the target-stage policy on installations whose API has no snapshot object. The waiver documents itself as honoured only on a stage that resolves and is mutable, and refused on immutable, production-like or unresolvable ones — but that question was asked only where a snapshot was attempted. On a snapshotless installation none is, so the final irreversibility gate was left reading the raw flag; and because no snapshot is ever taken there, every unreconciled overwrite on such an installation funnels the user into passing it. `overwrite_server_change`/`adopt_existing` combined with `allow_no_snapshot` could therefore overwrite a production or unknown stage irreversibly on an old on-prem build. The gate now evaluates the same policy — against the stage the snapshot branch resolved, not one re-derived from `parent_id`, so it cannot disagree with the decision made a few lines earlier — and a refusal says the flag was refused rather than asking for one that was already passed. A snapshotless installation with no overwrite waiver still pushes as before.
- Fix(mcp-server): `logout` distinguishes leftover credentials from leftover configuration. Any single `COREZOID_*` variable — `COREZOID_ACCOUNT_URL` alone — used to produce "environment variables still supply credentials", which sent the user hunting for a credential that was never set and drained the warning of meaning for the case where it is true. Only a token or a complete API-key pair now reports as still authenticated.
- Docs: the README no longer claims the HTTP transport runs without authentication when `COREZOID_HTTP_TOKEN` is unset. The server mints a random token at startup and prints it to stderr; every `/mcp` request needs it either way.
- Fix(mcp-server): `allow_no_snapshot` also waives a `CreateSnapshot` API failure, not only an unresolved `project_id`/`stage_id`. A transient platform error on the snapshot call left the push with no way forward at all — no flag combination could get past it — so an outage on one endpoint froze every push of an existing process. The waiver is gated on the same mutable, non-production stage policy as the resolution-failure case, and the failed call is still the safer default to retry (#169).
- Fix(mcp-server): `force` no longer waives two unrelated gates at once. It was the lint override AND the authorisation to overwrite a concurrent server change, so a `force` set for a lint finding — before any conflict existed — silently pre-authorised dropping another person's edit, with the impact report going nowhere but stderr. The concurrency gate now has its own waiver, `overwrite_server_change`, meant to be passed in reply to the block report rather than ahead of it; `force` is lint-only. The block report says so outright — anything written against the old contract arrives with `force` already set, and a report that never mentions the flag that was passed reads as a broken flag rather than a moved one.
- Fix(mcp-server): an authorised overwrite is now reported in the push result. `overwrite_server_change` and `adopt_existing` hand back what was dropped — the divergence, who last changed the process on the server, and an itemised list of the server changes the push discarded — instead of one line on stderr that an MCP host is free to discard, which left "Process deployed successfully" as the entire audit trail of a lost update. The record is rendered separately from the block report on purpose: that renderer describes a decision still to be taken ("what this push would commit", "no overlap, mergeable"), and the same wording in the record of a completed overwrite reads as if the server's changes were still on offer. `allow_active_stub_mode` and an `allow_no_snapshot` that permitted an irreversible overwrite are reported the same way, and lint findings waived by `force` are now labelled as overridden instead of appearing under "non-blocking".
...
- Fix(mcp-server): a scoped snapshot refusal no longer disables the pre-push rollback point. "Snapshots are disabled for this stage", "Snapshot access is disabled" and "Snapshots are unsupported for this project type" are policy and permission answers from a build that does implement snapshots; they were being read as proof that the installation has no snapshot object, cached as unsupported, and every later push against that project+stage then proceeded with no rollback point. A refusal naming a stage, project, user or access right is now an ordinary failure. Installation-wide wording ("Snapshots are disabled on this installation") is still recognised, so pushes are not blocked forever where the feature genuinely does not exist.
- Fix(mcp-server): `push-process` no longer overwrites an existing process when no pre-push snapshot could be taken because `project_id`/`stage_id` could not be resolved. An unresolved target is an unknown safety configuration, not a reason to skip the rollback point. New `allow_no_snapshot` waives it, and only on a stage that resolves and is mutable — never on immutable, production-like or unresolvable targets. Two cases are unaffected by design: a process with no deployed version (so `create-process` → `push-process` still works), and an installation whose API has no snapshot object at all — there was never a rollback point to take there, so those environments keep pushing existing processes with the usual notice, whether or not the target resolved.
- Fix(mcp-server): `push-process` no longer pushes a file with no pull baseline over a process that already has a deployed version — lost-update detection cannot run there, so the push was a blind overwrite. It now blocks and asks for a `pull-process`; new `adopt_existing` overrides it deliberately. Never-deployed processes proceed as before, and an unreadable server state fails closed. `allow_no_snapshot` and `adopt_existing` are both separate from `force` on purpose: `force` resolves a conflict you were shown, these waive a safety guarantee.
- Fix(mcp-server): when the post-deploy baseline or merge-ancestor refresh fails, `push-process` now says so in its result instead of only logging it. The deploy still succeeded, but the local concurrency state is stale, and reporting a bare success made the next push either re-flag the user's own change as a conflict or lose its 3-way ancestor.
...
- CI: The release now verifies its own published assets — all six `convctl` binaries, `checksums.txt`, `llms.txt`, `index.json` and `POWER.md` — and fails loudly if any is missing, because the launcher installs from exactly that asset list and otherwise falls back to requiring a local Go toolchain.
- CI: `govulncheck` is pinned to v1.7.0 instead of `@latest`, so a release gate cannot change behaviour between runs on its own.
