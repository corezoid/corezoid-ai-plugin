...
- Feat: **concurrent-change detection with 3-way merge** on `push-process`. `pull-process`/`pull-folder` now record the pulled server version (per-folder `.corezoid-baseline.json`) and a copy of the pulled scheme (`.corezoid-baseline/`, the merge ancestor). If someone else changed the process on the server between your pull and your push, the push **blocks** instead of silently overwriting their edits (a plain push runs `DeleteNotUsedNodes`, dropping any server node absent from your local copy). The block reports who last changed it on the server and when (author from the process commit list, falling back to the latest snapshot's `user_name`/time), the version divergence, and — per node — three short buckets: **your local edits** (what this push would commit), the **server's changes** since your pull, and the **overlap** where you and the server both touched the same node — then lets you choose. Non-overlapping server changes are **mergeable** (their new Code/API/condition edit or added node); overlapping ones are **true conflicts**. Resolve three ways: re-pull and re-apply; `merge=true` to graft the mergeable server changes into your file for review (conflicts kept as yours and listed, then push again) — a real 3-way merge that matches nodes by title and rewires links across the server's id regeneration; or `force=true` to overwrite (the existing auto-snapshot keeps the server version recoverable). Files with no baseline (never pulled) push normally with an advisory. Corezoid's API has no server-side optimistic lock, so this is a client-side read-compare-before-write; the auto-snapshot covers the small unavoidable TOCTOU window. Node bodies (including `api_code` source, conditions, options, links) round-trip through the export API and merge fully; nodes are matched across versions by title, or — for untitled nodes such as Start events and error finals — by obj_type + ordinal, so they merge 1:1 instead of colliding (a rename reads as delete+add).
