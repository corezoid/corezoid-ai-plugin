...
- Fix: CLI-mode OAuth sent an empty client_id (the account service answered with a foreign state, "OAuth state mismatch") — all three entry modes now share one `initOAuthClientID()`. `force=true` is unconditional again: it skips the silent refresh and discards the stored refresh token along with the session it belongs to.
- Feat: OAuth hardening per OAuth 2.1 / RFC 8414 / RFC 8252 — endpoints resolved from the account service's `/.well-known/oauth-authorization-server` metadata (falling back to the conventional `/oauth2/*` paths for older on-prem installs); silent token renewal via the `refresh_token` grant before any browser round-trip — the refreshed token is trusted only after one authenticated probe succeeds (the account service currently answers the grant with a session-scoped token the API rejects, verified live, so the flow falls back to the browser until the service mints API-grade tokens there; the refresh token itself is stored in `~/.corezoid/credentials`, removed by logout and included in its backup); token-endpoint responses are never echoed into error messages (only field names — a raw echo leaked a live token during testing); `ACCOUNT_URL` scheme validation (https required, plain http allowed for localhost only) before the browser opens; the callback listener is kept from port probe to serve (fixes a port-grab race and flaky logins); OAuth state widened to 256 bits.
- Feat: `status` tool — no-auth self-diagnosis (version, uptime, pid, config with an admin-host warning on `ACCOUNT_URL`, token presence/expiry, credentials/log paths; `probe=true` live-checks the token). Observability around server restarts: RFC3339 timestamps on every log line, startup banner and shutdown farewell (signals AND stdio EOF), a `run.sh` notice when the served binary changes (marker per binary path), and skill guards that tell the user to restart the session when tool handles die.
- Fix: token lifecycle. `login` validates an existing token with one authenticated probe (the workspace list — the account clients endpoint answers 200 even without a token, verified live, so it is only used to derive the API URL) before trusting it: an explicitly rejected token triggers a fresh OAuth flow, while transport trouble (5xx/network) keeps the working session instead of destroying it and popping a surprise browser. `force=true` re-authenticates unconditionally. After OAuth, a stale `ACCESS_TOKEN` in the loaded `.env` (resolved by the same walk-up `findAndLoadDotEnv` uses, not blindly `cwd/.env`) is REMOVED — not rewritten with the new secret, keeping tokens out of repo-local files — so `~/.corezoid/credentials` is the single token source. `logout` clears the in-memory session first (a failed file op can no longer leave a "logged-out" session alive), removes the token from the resolved `.env` (preserving other keys, matching indented lines like `loadDotEnv` does), and deletes the credentials file entirely instead of leaving a 0-byte husk.
