...
- Feat: **concurrent-change detection with 3-way merge** on `push-process`. `pull-process`/`pull-folder` record the pulled server version and merge ancestor. A later push blocks when the server changed, reports local/server/overlap buckets, and supports re-pull, `merge=true` for a local reviewable merge, or an explicitly forced overwrite after attempting a server snapshot. Corezoid has no atomic compare-and-swap for process deploys, so this is a client-side read/compare/write guard and does not claim transactional isolation.
