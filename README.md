...
| `COREZOID_HTTP_PORT`       | No       | Activate the Streamable HTTP transport on this port (e.g. `8080`); the server then listens for MCP over HTTP instead of stdio. Bound to loopback (`127.0.0.1`) by default — override with `COREZOID_BIND_ADDR`. The endpoint **has no built-in authentication**: credentials are loaded once from your saved OAuth credentials or env vars and shared across all requests, and the browser OAuth login flow is not available. Intended for local coding-agent integrations only. For hosted deployment, place it behind a reverse proxy (Nginx / Caddy / Cloudflare Access) that terminates TLS, enforces authentication and applies rate limiting — the MCP server does not replace those components |
| `COREZOID_BIND_ADDR`       | No       | Host the HTTP transport binds to (default `127.0.0.1`; `::1` and `localhost` are also accepted). Because the endpoint **has no built-in authentication**, any other value is refused at startup unless you also set `COREZOID_ALLOW_UNAUTHENTICATED_REMOTE=yes-i-know-there-is-no-auth` — binding to a routable interface exposes full Corezoid API access to anyone who can reach the port |
| `COREZOID_HTTP_ALLOWED_ORIGINS` | No  | Comma-separated browser origins allowed to read HTTP-transport responses, or `*` for any. Unset by default, which is the safe setting: no `Access-Control-Allow-Origin` is sent, so a web page cannot read replies. This controls readability only — it does not stop a page from *issuing* a request to the loopback port, so leave it unset unless a specific browser-based client needs it |
