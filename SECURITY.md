...
All credentials and per-workspace config live in user-level files under `~/.corezoid/`:
...
| `~/.corezoid/config-<name>.json` | Optional additional config files with the same schema as `config.json`. Read and merged on top of it (`config.json` wins on a duplicate `root_path`; among `config-*.json` the alphabetically first wins). Never created by the MCP server — only written back to when they own the matched workspace (token refresh, stage selection, explicit `logout`); the automatic abandoned-workspace cleanup never modifies them. | User |
...
**Concurrency:** writes go through an in-process mutex + a cross-process `flock` on every participating config file (`~/.corezoid/config.json.lock` and one `.lock` per `config-<name>.json`), followed by an atomic temp-file + rename. Two MCP-server processes started from different IDE windows serialise cleanly.
...
- `~/.corezoid/config.json` / `~/.corezoid/config-<name>.json` (and their `.lock` sidecars) — user-scope only, never in a repo
